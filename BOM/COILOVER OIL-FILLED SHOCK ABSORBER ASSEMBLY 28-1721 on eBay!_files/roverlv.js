...
if(!this._ebayLVTr_clicksBuilder||this._ebayLVTr_clicksBuilder=="")this._ebayLVTr_called=1;};var fieldTimes=new Array();var timeSpent=new Array();var lfs=null;var issubmit=false;var tsf="";var formTrack=false;this.enableFormTracking=function(){formTrack=true;var iframes=document.getElementsByClassName("trackiframe");for(i=0;i<iframes.length;i++){var iframe=iframes[i];if(iframe.addEventListener){iframe.addEventListener("focus",addOnFocus,false);iframe.addEventListener("blur",addOnBlur,false);}else if(iframe.attachEvent){iframe.attachEvent("onfocus",addOnFocus,false);iframe.attachEvent("onblur",addOnBlur,false);}}
var imgtrack=document.getElementsByClassName("trackImg");for(i=0;i<imgtrack.length;i++){var img=imgtrack[i];if(img.addEventListener){img.addEventListener("focus",addOnFocus,false);img.addEventListener("blur",addOnBlur,false);}else if(img.attachEvent){img.attachEvent("onfocus",addOnFocus,false);img.attachEvent("onblur",addOnBlur,false);}}
var formtrack=document.getElementsByClassName("formTrack");if(formtrack!='undefined'&&formtrack.length>0){var elems=document.getElementsByClassName("formTrack")[0].elements;for(i=0;i<elems.length;i++){var elem=elems[i];if(elem.type!="submit"){if(elem.addEventListener){elem.addEventListener("focus",addOnFocus,false);elem.addEventListener("blur",addOnBlur,false);}else if(iframe.attachEvent){elem.attachEvent("onfocus",addOnFocus,false);elem.attachEvent("onblur",addOnBlur,false);}}else{var classnam=elem.className;var index=classnam.indexOf("formsubmit");if(index>=0){elem.onclick=this.addOnsubmit;}}}};}
this.addOnFocus=function(){fieldTimes[this.name]=new Date().getTime();};this.addOnBlur=function(){lfs=this.name;var ftime=fieldTimes[lfs];var tmspnt=new Date().getTime()-ftime;if(timeSpent[lfs]!='undefined'&&timeSpent[lfs]!=null){timeSpent[lfs]=timeSpent[lfs]+tmspnt;}else{timeSpent[lfs]=tmspnt;}};this.addOnsubmit=function(){for(var sIndex in timeSpent){tsf=tsf+sIndex+"|"+timeSpent[sIndex]+";";}
if(tsf!=""){tsf=tsf.substring(0,tsf.length-1);issubmit=true;}};}
...
return true;};this.computeMaxVerticalScrollOffset=function(){var g=document.body.scrollTop;if(g>_svo){_svo=g;}
...
window.onunload=ebayLVTrClk.generateHeatMapRoverImp;window.onscroll=ebayLVTrClk.computeMaxVerticalScrollOffset;this.heatMapEnabled=1;}
